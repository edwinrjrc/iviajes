...
public class UtilIvDto {
	...
	public static String escribeObjetoEnLog(Object objeto) throws IvServiceRentCarException {
		try {
	...
	public static Date parseStringADate(String cadenaFecha, String patron, TimeZone timeZone) throws IvServiceRentCarException {
		try {
			if (StringUtils.isBlank(cadenaFecha)) {
				throw new IvServiceRentCarException("La fecha a convertir esta en blanco o es nula");
			}
	...
	public static String parseDateAString(Date fecha, String patron) throws IvServiceRentCarException {
		if (fecha == null) {
			throw new IvServiceRentCarException("La fecha es nula");
		}
		SimpleDateFormat sdf = new SimpleDateFormat((StringUtils.isBlank(patron)?PATRON_SDF_DEFECTO:patron));
	...
	public static BigDecimal redondea(BigDecimal valor, int numeroDecimales) throws IvServiceRentCarException {
		try {
			if (valor != null) {
				return valor.setScale(numeroDecimales, RoundingMode.HALF_UP);
			}
			else {
				throw new IvServiceRentCarException("Valor en nulo");
			}
		} catch (NullPointerException e) {
			throw new IvServiceRentCarException("Error: valor nulo", e);
		} catch (Exception e) {
			throw new IvServiceRentCarException(e.getMessage(), e);
		}
	}
	
	public static BigDecimal redondeA2(BigDecimal valor) throws IvServiceRentCarException {
		return redondea(valor,2);
	}
	
	public static BigDecimal redondeA3(BigDecimal valor) throws IvServiceRentCarException {
		return redondea(valor,3);
	}
	
	public static BigDecimal redondeA4(BigDecimal valor) throws IvServiceRentCarException {
		return redondea(valor,4);
	}
	
	public static BigDecimal redondeA5(BigDecimal valor) throws IvServiceRentCarException {
		return redondea(valor,5);
	}
	
	public static BigDecimal redondeA6(BigDecimal valor) throws IvServiceRentCarException {
